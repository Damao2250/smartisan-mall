...
const uploadMulter = require('./../util/upload')
...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        if (doc) {
            ...
            // 可以注册
            User.insertMany({
                avatar: `https://dummyimage.com/168x168/c7c7c7/fff.png&text=${userName}`,
                name: '新用户',
                cartList: [],
                orderList: [],
                addressList: [],
...
// 上传图片到本地
router.post('/uploadFile', uploadMulter.single('file'), async (req, res, next) => {
    try {
            let path = req.file.path.split('public')[1]
            let imgUrl = 'http://127.0.0.1:3333' + path
            res.json({
                status: '0',
                result: {
                    path: imgUrl,
